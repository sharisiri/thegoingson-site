...
const client_cred =
  'BQDJxs_XdlrSvwp4ngymJ7aCqGBuxP0kFkul11e8JQWfWknZOWmoSCIJOoNpcGw-hOfchBhdnSEBSzrNyvg';
var plObjects = [];
(async () => {
  try {
    var accessToken = await got.post('https://accounts.spotify.com/api/token', {
      form: true,
      body: { grant_type: 'client_credentials' },
      headers: { Authorization: 'Basic ' + client_cred },
    });
    accessToken = JSON.parse(accessToken.body).access_token;
    console.log(accessToken);
    var playlists = await got(
      'https://api.spotify.com/v1/users/ombudsmannen/playlists?limit=50',
      { json: true, headers: { Authorization: 'Bearer ' + accessToken } }
    );
    ...
    for (x in playlistItems) {
      if (playlistItems[x].name.startsWith('the goings on #')) {
        ...
        var pl = await got(playlistItems[x].href + '?market=SE', {
          json: true,
        ...
        plObj.image =
          'img/' + plObj.name.replace(/\s/g, '-').replace('#', '') + '.jpg';
        await sharp(plObj.image, { progressive: true })
          .resize(960, 960, { fit: 'inside', withoutEnlargement: true })
          .toFile('site/' + plObj.image);
        plObj.curator = new RegExp(/curated by ([\s\S]*?)\./g).exec(
          pl.body.description
        )[1];
        plObj.tracks = [];
        plObj.duration = 0;
        ...
        for (y in tracks) {
          ...
          track.artist = tracks[y].track.artists
            .map(function (elem) {
              return elem.name;
            })
            .join(', ');
          ...
          if (!track.preview) {
            console.log(
              track.artist + '-' + track.title + ' ' + track.preview + '\n\n'
            );
          ...
          track.youtubeLink =
            ...
            track.artist.replace(/\s/g, '+').replace('/,/g', '') +
            '+' +
            track.title.replace(/\s/g, '+');
          plObj.tracks.push(track);
        }

        noPrevKeys = Object.keys(noPreviewTracks);
        noPrevTrksData = await got(
        ...
        for (z in noPrevTrksData.body.tracks) {
          if (noPrevTrksData.body.tracks[z].preview_url) {
            ...
            plObj.tracks[noPreviewTracks[noPrevKeys[z]]].preview =
              noPrevTrksData.body.tracks[z].preview_url;
          }
        }
